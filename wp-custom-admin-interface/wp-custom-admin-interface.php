...
function wp_custom_admin_interface_settings_page_content ($submenu) {
    ...
    <div class="wrap">
        <div id="poststuff">
            ...
            <!--pro message-->
            <div style="border-radius: 4px; background-color: #ef4534;" data-dismissible="disable-done-notice-forever" class="notice is-dismissible custom-admin-interface-pro-notice" data-nonce="<?php echo wp_create_nonce( 'dismiss-pro-notice' ); ?>">
    ...
    <table class="form-table">       
        ...
        if($submenu !== 'ManageSettings' && $submenu !== 'Help') {
            ...
            $noonce = wp_create_nonce( 'delete_transients' );

            echo '<button data-noonce="'.$noonce.'" type="submit" name="submit" id="submit" class="button button-primary wp-custom-admin-interface-save-button"><i class="fa fa-check-square" aria-hidden="true"></i> ';
...
function wp_custom_admin_interface_register_admin($hook)
{
    ...
    if(in_array($hook, $wp_custom_admin_interface_settings_pages)) {
        ...
        wp_enqueue_script( 'custom-admin-script', plugins_url( '/inc/adminscript.js', __FILE__ ), array( 'jquery','wp-color-picker' ),wp_custom_admin_interface_get_version());
    ...
    if('admin.php' == $pagenow && isset($_GET['page']) && $_GET['page'] == 'wpcai_admin_menu'){
        ...
        wp_enqueue_script('menu-page', plugins_url( '/inc/options/options-page-admin-menu.js', __FILE__ ), array('jquery'),wp_custom_admin_interface_get_version()); 
    }
    ...
    if('admin.php' == $pagenow && isset($_GET['page']) && ($_GET['page'] == 'wpcai_admin_menu' || $_GET['page'] == 'wpcai_admin_toolbar')){
        ...
        //load older version of jquery ui sortable
        wp_deregister_script( 'jquery-ui-sortable'  );
        wp_register_script('jquery-ui-sortable', plugins_url('/inc/external/sortable.min.js', __FILE__ ), array('jquery-ui-mouse' ),wp_custom_admin_interface_get_version(),true);
    ...
    if('admin.php' == $pagenow && isset($_GET['page']) && $_GET['page'] == 'wpcai_hide_meta'){
        wp_enqueue_script('hide-meta', plugins_url( '/inc/options/options-page-hide-meta.js', __FILE__ ), array('jquery'),wp_custom_admin_interface_get_version()); 
    }
...
function wp_custom_admin_interface_import_settings() {
    ...
    if (strpos($settings, ':') !== false) {
        $extractedSettings = unserialize($settings, array('allowed_classes' => false));
    } else {
        $extractedSettings = unserialize(base64_decode($settings), array('allowed_classes' => false));
    }
...
function wp_custom_admin_interface_delete_transients() {
    ...
    if( current_user_can( 'manage_options') ){
        ...
        $noonce = sanitize_text_field( $_POST['noonce'] );
        $noonce_check = wp_verify_nonce( $noonce, 'delete_transients' );
...
function wpcai_pro_notice_disable() {
    ...
    if ( current_user_can( 'manage_options') ){
        ...
        $noonce_check = wp_verify_nonce( $noonce, 'dismiss-pro-notice' );
